...
class ConsultasHTTP {
  ...
  Future<Barberia> consultarBarberia(String id) async {
    dynamic responseHttp;
    final link = Uri.parse(BaseUrl + "api/consultar_barberia.php?id="+id);
    final response = await http.get(link);
    Barberia barberia;
    ...
    if (response.statusCode != 200) {
      ...
      var json = convert.jsonDecode(response.body);
      responseHttp = json["barberia"];
      barberia = new Barberia(
        responseHttp["id"],
        responseHttp["nombre"],
        responseHttp["direccion"],
        responseHttp["ciudad"],
        responseHttp["logo"],
        responseHttp["horario"],
        responseHttp["telefono"],
        responseHttp["calificacion"],
        double.parse(responseHttp["Lat"]),
        double.parse(responseHttp["Lng"]),
        responseHttp["horarioa"],
        responseHttp["horarioc"]);
      }
    return barberia;
  }
  ...
  Future<List<CitaModel>> listarCitasBarbero(String id) async {
    dynamic responseHttp;
    List<CitaModel> citas = [];
    final link = Uri.parse(BaseUrl + "api/citas_barbero.php?id_barbero=" + id);
    final response = await http.get(link);
    if (response.statusCode != 200) {
      responseHttp = 0;
    } else {
